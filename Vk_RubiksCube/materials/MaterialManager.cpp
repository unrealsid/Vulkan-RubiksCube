...
void material::MaterialManager::init()
{
    ...
    utils::MemoryUtils::allocate_buffer_with_mapped_access(device_manager->getAllocator(), material_buffer_size, material_params_buffer);
    material_params_address = utils::MemoryUtils::getBufferDeviceAddress(device_manager->getDispatchTable(), material_params_buffer.buffer);
    
    //Setup Texture descriptors
    utils::DescriptorUtils::setupTextureDescriptors(device_manager->getDispatchTable(), textures, texture_descriptor_layout, texture_descriptor_set);

    init_shaders();
}

void material::MaterialManager::add_texture(const Image& texture)
{
...
void material::MaterialManager::init_shaders()
{
    ...
    for (const auto& shader : shader_info)
    {
        ...
        shader_object->create_shaders(device_manager->getDispatchTable(),
